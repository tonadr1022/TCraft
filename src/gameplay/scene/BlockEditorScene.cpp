...
void BlockEditorScene::Render(Renderer& renderer, const Window& window) {
  ...
  renderer.SubmitChunkDrawCommand(
      glm::rotate(glm::translate(glm::mat4{1}, glm::vec3{0, 0, 0}), block_rot_, glm::vec3{0, 1, 0}),
      add_model_blocks_[static_cast<uint32_t>(add_model_type_)].mesh.handle_);
...
void BlockEditorScene::OnImGui() {
  ...
  if (ImGui::CollapsingHeader("Add Model", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::Button("Save")) {
      ...
      if (add_model_type_ == BlockModelType::All) {
        ...
        j["textures"] = {
            {"posx", add_model_data_unique_.tex_pos_x}, {"negx", add_model_data_unique_.tex_neg_x},
            {"posy", add_model_data_unique_.tex_pos_y}, {"negy", add_model_data_unique_.tex_neg_y},
            {"posz", add_model_data_unique_.tex_pos_z}, {"negz", add_model_data_unique_.tex_neg_z},
        };
      }
  ...
  if (all_block_model_names_.size() > 0) {
    if (ImGui::CollapsingHeader("Edit Model", ImGuiTreeNodeFlags_DefaultOpen)) {
